...
#close(RESULT_FILE);
my $missing_ENST_file = $data_dir.'missing/missingENSEMBLtranscript.';
my $missing_GeneName_file = $data_dir.'missing/missingGeneaname.';
my $missing_entrez_file = $data_dir.'missing/missingEntrez.';
my $missing_ENSG_file = $data_dir.'missing/missingENSG.';
...
sub refseq2ensembl(){
    ...
    while(@all){
        ...
        chomp($line);
        if($line){
            my($refseq_IDs,$ensembl_tr)= split("\t",$line);
            my @refseqIDs=split(",",$refseq_IDs);
            print $refseqIDs[0];
            $ensembl_tr2refseq_hash{$ensembl_tr}=\@refseqIDs;
            foreach my $refseq_ID (@refseqIDs){            
    ...
    while(@all){
        ...
        if($line){
            ...
            if (not exists $ensembl_tr2refseq_hash{$enst}){
                ...
                if(!$refseq_mRNA eq ''){
                    push @{$ensembl_tr2refseq_hash{$enst}},$refseq_mRNA;
                    $refseq2ensembl_tr_hash{$refseq_mRNA}=$enst;
                }else{
                    push @{$ensembl_tr2refseq_hash{$enst}},$refseq_predicted;
    ...
    while(@all){
        ...
        if ($line){            
            ...
            if ($ens_transcript2uni_hash{$ensembl_transcript_id}) {#use ensembl_transcript_id for mapping
                my @tmp_uni_id = @{$ens_transcript2uni_hash{$ensembl_transcript_id}};#revised
                push @uni_id, @tmp_uni_id;
                }
                else{
                    print"can't find corresponding UNI_ID for $ensembl_transcript_id\n";
                    open (MISSING_GENE,'>>'.$missing_ENST_file.$tax_ID)||die "Could not open file $!";#altered
    ...
    while(@all){
        ...
        if($line){
            ...
            if(exists $gene_symbol2uni_hash{$gene_symbol}){
                ...
                open (MISSING_GENE,'>>'.$missing_GeneName_file.$tax_ID)||die "Could not open file $!";#altered
    ...
    while(@all){
        ...
        if($line){
            ...
            if(exists $gene_symbol2uni_hash{$gene_symbol}){#check if the genesymbol correspondent NMs 
                ...
                open (MISSING_GENE,'>>'.$missing_GeneName_file.$tax_ID)||die "Could not open file $!";
                print MISSING_GENE "$gene_symbol\n";
                close(MISSING_GENE);
        ...
        if($line){
            ...
            if(exists $ensembl_tr2refseq_hash{$ensembl_transcript_ID}){
                #######################====BUG:there are many old ensembl transcript ID need to be transformed to check if they retired or have successor by ensembl API.##########################################
                   ...
                   my @refseq_IDs = @{$ensembl_tr2refseq_hash{$ensembl_transcript_ID}};
                    ...
                    if($criter_1>=$creterion1 and $criter_2>=$creterion2 and $criter_3>=$creterion3){
                        ...
                        print "$line\n";
                        open (MISSING_GENE,'>>'.$missing_ENST_file.$tax_ID)||die "Could not open file $!";
                        print MISSING_GENE "$ensembl_transcript_ID\n";
                        close(MISSING_GENE);
    ...
    while(@all){
        ...
        if($line){     
            ...
            if(exists $ensembl_tr2refseq_hash{$transcriptid}){
                @refseq_ids= @{$ensembl_tr2refseq_hash{$transcriptid}};#this line still need review to ensure that every ensembl_tr can find a refseq id##==BUG
            }else{
                print "$transcriptid not exist!\n";
                open (MISSING_ENST,'>>'.$missing_ENST_file.$tax_ID)||die "Could not open file $!";
                print MISSING_ENST "$transcriptid\n";
                close(MISSING_ENST);
            }
            if($miTGscore>=$cutoff and @refseq_ids and $mirna_accession){#if this line passed the quality control
                foreach my $refseq_id (@refseq_ids){
...
sub mirtarbase(){
    ...
    my $missing_GeneName_file=  $data_dir.'mirtarbase/mirtarbase_missinggenename.txt';
    ...
    while(@all){
        ...
        if($line){
            ...
            if((!exists $gene_symbol2uni_hash{$target_Gene}) and (! exists $entrez2uni_hash{$target_Entrez})){
                open (MISSING_GENE,'>>'.$missing_GeneName_file.$tax_ID)||die "Could not open file $!"; 
                open (MISSING_ENTREZ,'>>'.$missing_entrez_file.$tax_ID)||die "Could not open file $!"; 
                print MISSING_GENE "$target_Gene\n";
                print MISSING_ENTREZ "$target_Entrez\n";
                print "MIRTARBASE can't find refseqs : $target_Gene, $target_Entrez\n";
                close(MISSING_GENE);
                close(MISSING_ENTREZ);
                 next;
           }else{
...
sub mirtar(){
    ...
    my $tax_ID = "10090";# by default genes are from mouse
    if ($species eq "human"){$tax_ID = "9606";}
    #  my $missing_GeneName_file=  $data_dir.'/mirtar/'.'mirtar_missinggenename.txt';
    my %gene_symbol2uni_hash=%{retrieve($data_dir.'annotations/'.$tax_ID.'_rnaGeneName2refseqs.hash')};    
    my %mirAlias2Accession_hash=%{retrieve($data_dir.'miRBase/mirAlias2Accession.hash')};
    my %entrez2uni_hash=%{retrieve($data_dir.'annotations/'.$tax_ID.'_enterz2refseqs.hash')};
    ...
    while(@all){
        ...
        if($line){
            ...
            foreach my $gene_symbol (@gene_symbols){
                if (exists $gene_symbol2uni_hash{$gene_symbol}){push @refseq_ids, @{$gene_symbol2uni_hash{$gene_symbol}};}else{
                open (MISSING_GENE,'>>'.$missing_GeneName_file.$tax_ID)||die "Could not open file $!"; 
                print MISSING_GENE "$gene_symbol\n";                }                          
            }
    ...
    while(@all){
        ...
        if($line){
            ...
            if(!exists $ensg2refseqs_hash{$ensg_id}){
                 print "PACCMIT can't find refseqs : $ensg_id\n";
                open (MISSING_GENE,'>>'.$missing_ENSG_file.$tax_ID)||die "Could not open file $!"; 
                print MISSING_GENE "$ensg_id\n";             
    ...
    while(@all){
        ...
        if($line){
            ...
            if(!exists $ensembl_tr2refseq_hash{$enst_id}){
               ...
               @refseq_ids=@{$ensembl_tr2refseq_hash{$enst_id}};          
            }#else
...
#&refseq2entrez('human');
#&refseq2entrez('mouse');
#&microcosm('human');#sueing the ensembl transcriptID 2 refseq ID hash, problem
#&microcosm('mouse');
#&targetscan('human','broadly');#using the refseq ids there is no problem at all
#&targetscan('human','conserved');
#&targetscan('human','nonconserved');
#&targetscan('mouse','broadly');
#&targetscan('mouse','conserved');
#&targetscan('mouse','nonconserved');
    #############################################
#&pictar('mouse','most_conserved');#this file match gene names to refseq_ID there are missing file for this
...
#&pita('human','no_flank','TOP');#there is no problem at all; it use refsq IDs
#&pita('human','flank','TOP');
#&pita('human','no_flank','ALL');
#&pita('human','flank','ALL');
#&pita('mouse','no_flank','TOP');
...
#&microrna_org('human','conserved');#no problem at all, it use refseq IDs
#&microrna_org('human','non_conserved');
#&microrna_org('mouse','conserved');
#&microrna_org('mouse','non_conserved');
    #########################finished
#&mirdb('human');#no problem use refseqs
#&mirdb('mouse');
    ############finished###########
#&mirnamap('human',1,1,1);#problem, using ensembl transcript ID
#&mirnamap('human',1,2,1);
#&mirnamap('human',2,1,1);
#&mirnamap('human',2,2,1);
#&mirnamap('mouse',1,1,1);
...
#&mirnamap('mouse',2,2,1);
########################################
#&reptar('human');#using default parameters, usring refseq Id no problem
#&reptar('mouse');
############################
#&eimmo('human',undef,undef,'CDS');#no problm  should be fine
#&eimmo('mouse',undef,undef,'');
...
#&microT4('mouse','microtv4',0);#mouse, microt4,0.7
#&microT4('human',undef,0);#human,micro4,0.7
#&microT4('mouse','CDS',0);#mouse,cds microt, 0.7
#&microT4('human','CDS',0);
##############################
#&mirtarbase();#problem, using genename and entrez id ,missing quite a lot
#&mirtarbase('human',undef);
#&mirtarbase(undef,'SE');
#&mirtarbase('human','SE');
#########################################
#&mirtar('3utr');#problem, using genename and entrez id ,missingquite a lot
#&mirtar('5utr');
#&mirtar('all');
#&mirtar('cds');
##################################
##&targetrank('human',1); no problem it has refseq ID
##&targetrank('human',0);
##&targetrank('mouse',1);
##&targetrank('mouse',0);
&targetspy('human','sens','seed');
#####################################
&targetspy('human','sens','all');#no problem it has refseq id
&targetspy('human','spec','seed');
&targetspy('mouse','sens','seed');
&targetspy('mouse','sens','all');
&targetspy('mouse','spec','seed');
#&tarbase('human');#need runing again, since it using gene name
#&tarbase('mouse');
#need running again use ENST
#&paccmit('','_access','_2_5');#
#&paccmit('','','');#no filter
#&paccmit('','_access','');#access only filter
#&paccmit('_cons','','');#conservation filter only
