...
my $microcosm_detail_hash_file=$data_dir."microcosm/".$species.'_microcosm_mirna2target.detail_hash';    
store \%microcosm_detail_hash,$microcosm_detail_hash_file;
my $microcosm_hash_file="microcosm/".$species.'_microcosm_mirna2target.hash';
store \%microcosm_hash,$microcosm_hash_file;
return ($microcosm_hash_file,$microcosm_detail_hash_file);
}
...
sub targetspy(){
    ...
    $seed//='seed';#or no_seed
    my $targetspy_file = $data_dir.'targetspy/'.$species.'_'.$sensitivity.'_'.$seed.'_targetspy.tsv';
    my $tax_ID = "10090";# by default genes are from mouse
    if ($species eq "human"){$tax_ID = "9606";}
    my %mirAlias2Accession_hash=%{retrieve($data_dir.'miRBase/mirAlias2Accession.hash')};
...
######################HGNC_history###########################################################
    #this part is for the mapping of HGNC old symbol to the newest symbol because the TARBASE, since it contain many old Gene symbol
...
sub paccmit_cds(){#this program only have human data available , and different format for CDS an orgion
    ...
    my $species='human';
    $con//='_cons';# by default,take conservative or ''   
    my $paccmit_cds_file = $data_dir.'paccmit/paccmit_cds'.$species.$con.'.txt';
    my $tax_ID = "10090";# by default genes are from mouse
    if ($species eq "human"){$tax_ID = "9606";}
    my %ensembl_tr2refseq_hash=%{retrieve($data_dir.'annotations/'.$tax_ID.'_ensembl_transcriptID2refseqID2refseqID.hash')};
    my %mirAlias2Accession_hash=%{retrieve($data_dir.'miRBase/mirAlias2Accession.hash')};
    ...
    while(@all){
        ...
        if($line){
            ...
            if(!exists $ensembl_tr2refseq_hash{$enst_id}){
                 print "can't find refseqs : $enst_id\n";
                 next;
           }else{
...
=begin  BlockComment  # BlockCommentNo_1 PACCMIT and TARBASE not complete
...
&paccmit();
&paccmit('','','');
&paccmit('_cons','_access','');#$con if it is conservatived, $considertiong accessiblity or not $location restriction 2-5
